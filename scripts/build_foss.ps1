...
if ($BuildType -eq "apk" -or $BuildType -eq "both") {
    ...
    if ($LASTEXITCODE -eq 0) {
        $apkFile = Get-ChildItem "build\app\outputs\apk\release\*.apk" | Select-Object -First 1
        Write-Host "✅ FOSS APK built successfully!" -ForegroundColor Green
        Write-Host "   Output: $($apkFile.FullName)" -ForegroundColor Cyan
    } else {
...
if ($BuildType -eq "appbundle" -or $BuildType -eq "both") {
    ...
    if ($LASTEXITCODE -eq 0) {
        $aabFile = Get-ChildItem "build\app\outputs\bundle\release\*.aab" | Select-Object -First 1
